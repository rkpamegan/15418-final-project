...
Mesh::VertexRef Mesh::emplace_vertex() {
	VertexRef vertex;
...
Mesh::EdgeRef Mesh::emplace_edge(bool sharp) {
	EdgeRef edge;
	if (free_edges.empty()) {
		//allocate a new edge:
...
Mesh::FaceRef Mesh::emplace_face(bool boundary) {
	FaceRef face;
	if (free_faces.empty()) {
		//allocate a new face:
...
Mesh::HalfedgeRef Mesh::emplace_halfedge() {
...
void Mesh::erase_vertex(VertexRef v) {
	//clear data:
	v->id |= 0x80000000u;
...
void Mesh::erase_edge(EdgeRef e) {
	//clear data:
	e->id |= 0x80000000u;
	e->sharp = false;
...
void Mesh::erase_face(FaceRef f) {
	//clear data:
	f->id |= 0x80000000u;
	f->boundary = false;
...
void Mesh::erase_halfedge(HalfedgeRef h) {
	//clear data:
	h->id |= 0x80000000u;
...
bool Mesh::Vertex::on_boundary() const {
	bool bound = false;
...
uint32_t Mesh::Vertex::degree() const {
	uint32_t d = 0;
	HalfedgeCRef h = halfedge;
	do {
		d++;
...
Vec3 Mesh::Vertex::neighborhood_center() const {
	Vec3 c;
...
float Mesh::Vertex::angle_defect() const {
...
float Mesh::Vertex::gaussian_curvature() const {
...
uint32_t Mesh::Face::degree() const {
	uint32_t d = 0;
	HalfedgeCRef h = halfedge;
	do {
		d++;
...
bool Mesh::Edge::on_boundary() const {
	return halfedge->face->boundary || halfedge->twin->face->boundary;
}


Vec3 Mesh::Edge::normal() const {
...
uint32_t Mesh::id_of(ElementCRef elem) {
	return std::visit( [&](auto ref) -> uint32_t { return ref->id; }, elem );
}

Vec3 Mesh::normal_of(Mesh::ElementCRef elem) {
	Vec3 n;
...
Vec3 Mesh::center_of(Mesh::ElementCRef elem) {
	Vec3 pos;
...
uint32_t Mesh::id_of(ElementRef elem) { return id_of(const_from(elem)); }
Vec3 Mesh::center_of(ElementRef elem) { return center_of(const_from(elem)); }
Vec3 Mesh::normal_of(ElementRef elem) { return normal_of(const_from(elem)); }

Mesh::ElementCRef Mesh::const_from(ElementRef elem) {
	return std::visit( [&](auto r) { return ElementCRef(r); }, elem );
}


//count boundaries:
size_t Mesh::n_boundaries() const {
	size_t n = 0;
...
template< typename T >
static std::unordered_set< T const * > element_addresses(std::vector< T > const &list) {
...
std::optional<std::pair<Mesh::ElementCRef, std::string>> Mesh::validate() const {
...
/*
Mesh Mesh::from_indexed_mesh_with_corner_data(
...
{ 
	...
	Mesh mesh = Mesh::from_indexed_faces(indexed_vertices, 
			indexed_faces, indexed_normals, indexed_uvs, normals_, uvs_);

	return mesh;
}

Mesh Mesh::from_indexed_mesh(Indexed_Mesh const &indexed_mesh) {
	...
	//build halfedge mesh with the extracted vertex/face data:
	Mesh mesh = Mesh::from_indexed_faces(indexed_vertices, indexed_faces);
