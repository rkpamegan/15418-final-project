...
// public:
// 	/*
// 	 * A Mesh is comprised of four atomic element types:
...
// 	    a constant reference or pointer to a Mesh.  Since these types are
// 	    used so frequently, we will use "CRef" as a shorthand abbreviation for
// 	    "constant iterator."
// 	*/
// 	using VertexCRef = std::vector<Vertex>::const_iterator;
// 	using EdgeCRef = std::vector<Edge>::const_iterator;
// 	using FaceCRef = std::vector<Face>::const_iterator;
// 	using HalfedgeCRef = std::vector<Halfedge>::const_iterator;
...
// 	//static Mesh from_indexed_faces(std::vector< Vec3 > const &vertices, std::vector< std::vector< Index > > const &faces);
// 	static Mesh from_indexed_faces(std::vector< Vec3 > const &vertices, 
...
// 	static Mesh from_indexed_mesh_with_corner_data(
...
// 		std::vector<Vec3> const &normals = std::vector<Vec3>{}, 
// 		std::vector<Vec2> const &uvs = std::vector<Vec2>{});
// 	static Mesh from_indexed_mesh(const Indexed_Mesh& mesh);

// 	//generate a cube:
// 	static Mesh cube(float cube_radius);

// 	//DEBUG: so test cases compile -- but this is going away:
// 	//TODO: remove this constructor:
// 	Mesh(std::vector< std::vector< Index > > const &faces, std::vector< Vec3 > const &vertices) {
// 		*this = from_indexed_faces(vertices, faces);
// 	}

// 	// You can create a mesh by moving data from an existing mesh:
// 	//TODO: needed?
// 	Mesh(Mesh&& src) = default; //take ownership of contents of existing mesh
...
// 	std::vector<Face> free_faces;
// 	std::vector<Halfedge> free_halfedges;
...
// 	inline bool operator<(Mesh::Ref const & i, Mesh::Ref const & j) { \
// 		return &*i < &*j; \
// 	}
...
// 			} \
// 		}
// 	HASH_BY_ADDRESS( Mesh::VertexRef );
// 	HASH_BY_ADDRESS( Mesh::EdgeRef );
// 	HASH_BY_ADDRESS( Mesh::FaceRef );
// 	HASH_BY_ADDRESS( Mesh::HalfedgeRef );
// 	HASH_BY_ADDRESS( Mesh::VertexCRef );
// 	HASH_BY_ADDRESS( Mesh::EdgeCRef );
// 	HASH_BY_ADDRESS( Mesh::FaceCRef );
// 	HASH_BY_ADDRESS( Mesh::HalfedgeCRef );
